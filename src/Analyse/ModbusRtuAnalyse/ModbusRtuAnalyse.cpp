...
void ModbusRtuAnalyse::AnalyseFunc(const std::string& msg, const nextFrame& nextframe)
{
    ...
    {
        analyseFinishCallback_(resOK, resRW);
    }
}
...
bool ModbusRtuAnalyse::HandleData(const frame& v_data, const nextFrame& nextframe)
{
    if(v_data.empty())
    {
        return false;
...
std::string ModbusRtuAnalyse::HandleData_typeBit(const char& data, const int& bit)
{
