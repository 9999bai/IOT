...
{
    modbustcpAnalysePtr_->setAnalyseFinishCallback(std::bind(&ModbusTcpMediator::HandleAnalyseFinishCallback, this, std::placeholders::_1, std::placeholders::_2));
...
void ModbusTcpMediator::HandleAnalyseFinishCallback(bool ok, enum_RW rw)
{
    if(rw == enum_write)
    {
        if(ok)
